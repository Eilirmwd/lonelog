...
#strong[Numbering:] Start at `Rd1` each combat. Rounds are local to the encounter, not the session.
...
#strong[Q: Should I track initiative order?] A: Only if your system uses it and it matters. Most solo players go in logical order (threats first, then PC, then allies) or follow their system's rules. To note it: `Rd1 (Init: Captain 18, Alex 15, Jordan 12, Pirates 8)`.
...
#strong[Q: How does this work in an analog notebook?] A: Use `--- COMBAT ---` and `--- END COMBAT ---` as delimiters. Round markers (`Rd1`, `Rd2`) work as-is. For foe tags, write them in the margin or in a column beside the round text. Update HP with strikethroughs: #strike[HP 6] HP 3.
